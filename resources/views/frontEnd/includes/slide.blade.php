            ...
            <div class="widget">
                ...
                <ul class="cat">
                    @foreach($Categories as $Category)
                        ...
                        if ($Category->$slug_var != "" && \App\Helpers\Helper::GeneralWebmasterSettings("links_status")) {
                            if (App::getLocale() != env('DEFAULT_LANGUAGE')) {
                                $Category_link_url = url(App::getLocale() . "/" . $Category->$slug_var);
                            } else {
                                $Category_link_url = url($Category->$slug_var);
                            }
                        } else {
                            if (App::getLocale() != env('DEFAULT_LANGUAGE')) {
                                $Category_link_url = route('FrontendTopicsByCatWithLang', ["lang" => App::getLocale(), "section" => $Category->webmasterSection->name, "cat" => $Category->id]);
                            } else {
                        ...
                        @foreach($Category->fatherSections as $MnuCategory)
                            ...
                            if ($MnuCategory->$slug_var != "" && \App\Helpers\Helper::GeneralWebmasterSettings("links_status")) {
                                if (App::getLocale() != env('DEFAULT_LANGUAGE')) {
                                    $SubCategory_link_url = url(App::getLocale() . "/" . $MnuCategory->$slug_var);
                                } else {
                                    $SubCategory_link_url = url($MnuCategory->$slug_var);
                                }
                            } else {
                                if (App::getLocale() != env('DEFAULT_LANGUAGE')) {
                                    $SubCategory_link_url = route('FrontendTopicsByCatWithLang', ["lang" => App::getLocale(), "section" => $MnuCategory->webmasterSection->name, "cat" => $MnuCategory->id]);
                                } else {
            ...
            <div class="widget">
                ...
                <ul class="recent">
                    @foreach($TopicsMostViewed as $TopicMostViewed)
                        ...
                        if ($TopicMostViewed->$slug_var != "" && \App\Helpers\Helper::GeneralWebmasterSettings("links_status")) {
                            if (App::getLocale() != env('DEFAULT_LANGUAGE')) {
                                $topic_link_url = url(App::getLocale() . "/" . $TopicMostViewed->$slug_var);
                            } else {
                                $topic_link_url = url($TopicMostViewed->$slug_var);
                            }
                        } else {
                            if (App::getLocale() != env('DEFAULT_LANGUAGE')) {
                                $topic_link_url = route('FrontendTopicByLang', ["lang" => App::getLocale(), "section" => $TopicMostViewed->webmasterSection->name, "id" => $TopicMostViewed->id]);
                            } else {
                                    ...
                                    <a href="{{ $topic_link_url }}">
                                        ...
                                        @elseif($TopicMostViewed->webmasterSection->type==2 && $TopicMostViewed->video_file!="")
                                            @if($Topic->video_type ==1)
                                                <?php
                                                $Youtube_id = \App\Helpers\Helper::Get_youtube_video_id($Topic->video_file);
                                                ?>
                                                @if($Youtube_id !="")
                                            ...
                                            @elseif($Topic->video_type ==2)
                                                <?php
                                                $Vimeo_id = \App\Helpers\Helper::Get_vimeo_video_id($Topic->video_file);
                                                ?>
                                                @if($Vimeo_id !="")
                                                    <?php
                                ...
                                @foreach($SideBanners as $SideBanner)
                                    @if($SideBanner->youtube_link !="")
                                        @if($SideBanner->video_type ==1)
                                            <?php
                                            $Youtube_id = \App\Helpers\Helper::Get_youtube_video_id($SideBanner->youtube_link);
                                            ?>
                                            @if($Youtube_id !="")
                                                {{-- Youtube Video --}}
                                        ...
                                        @elseif($SideBanner->video_type ==2)
                                            <?php
                                            $Vimeo_id = \App\Helpers\Helper::Get_vimeo_video_id($SideBanner->youtube_link);
                                            ?>
                                            @if($Vimeo_id !="")
                                                {{-- Vimeo Video --}}
                                                <iframe width="100%" height="500" frameborder="0" allowfullscreen
