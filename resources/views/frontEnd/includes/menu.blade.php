    ...
    <ul class="nav navbar-nav">
        ...
        @foreach($HeaderMenuLinks as $HeaderMenuLink)
                        ...
                        <ul class="dropdown-menu">
                            @foreach($HeaderMenuLink->webmasterSection->sections as $MnuCategory)
                                @if($MnuCategory->father_id ==0)
                                    <li>
                                        <?php
                                        if ($MnuCategory->$slug_var != "" && \App\Helpers\Helper::GeneralWebmasterSettings("links_status")) {
                                        if (App::getLocale() != env('DEFAULT_LANGUAGE')) {
                                        $Category_link_url = url(App::getLocale()."/" .$MnuCategory->$slug_var);
                                        }else{
                                        $Category_link_url = url($MnuCategory->$slug_var);
                                        }
                                        } else {
                                        if (App::getLocale() != env('DEFAULT_LANGUAGE')) {
                                        $Category_link_url = route('FrontendTopicsByCatWithLang', ["lang"=>App::getLocale(),"section" => $HeaderMenuLink->webmasterSection->name, "cat" => $MnuCategory->id]);
                                        }else{
                        ...
                        <ul class="dropdown-menu">
                            @foreach($HeaderMenuLink->webmasterSection->topics as $MnuTopic)
                                @if($MnuTopic->expire_date =='' || ($MnuTopic->expire_date !='' && $MnuTopic->expire_date >= date("Y-m-d")))
                                    <li>
                                        <?php
                                        if ($MnuTopic->$slug_var != "" && \App\Helpers\Helper::GeneralWebmasterSettings("links_status")) {
                                        if (App::getLocale() != env('DEFAULT_LANGUAGE')) {
                                        $topic_link_url = url(App::getLocale()."/" .$MnuTopic->$slug_var);
                                        }else{
                                        $topic_link_url = url($MnuTopic->$slug_var);
                                        }
                                        } else {
                                        if (App::getLocale() != env('DEFAULT_LANGUAGE')) {
                                        $topic_link_url = route('FrontendTopicByLang', ["lang"=>App::getLocale(),"section" => $HeaderMenuLink->webmasterSection->name, "id" => $MnuTopic->id]);
                                        }else{
            ...
            @elseif($HeaderMenuLink->type==2)
                ...
                <li>
                    <?php
                    if ($HeaderMenuLink->webmasterSection->$slug_var != "" && \App\Helpers\Helper::GeneralWebmasterSettings("links_status")) {
                    if (App::getLocale() != env('DEFAULT_LANGUAGE')) {
                    $mmnnuu_link = url(App::getLocale()."/" .$HeaderMenuLink->webmasterSection->$slug_var);
                    }else{
                    $mmnnuu_link = url($HeaderMenuLink->webmasterSection->$slug_var);
                    }
                    }else{
                    if (App::getLocale() != env('DEFAULT_LANGUAGE')) {
                    $mmnnuu_link =url(App::getLocale()."/" .$HeaderMenuLink->webmasterSection->name);
                    }else{
            ...
            @elseif($HeaderMenuLink->type==1)
                ...
                // Direct Link
                if (App::getLocale() != env('DEFAULT_LANGUAGE')) {
                $this_link_url = url(App::getLocale()."/" .$HeaderMenuLink->link);
                }else{
                        ...
                        <ul class="dropdown-menu">
                            @foreach($HeaderMenuLink->subMenus as $subMenu)
                                            ...
                                            <ul class="dropdown-menu">
                                                @foreach($subMenu->webmasterSection->sections as $SubMnuCategory)
                                                    @if($SubMnuCategory->father_id ==0)
                                                        <li>
                                                            <?php
                                                            if ($SubMnuCategory->$slug_var != "" && \App\Helpers\Helper::GeneralWebmasterSettings("links_status")) {
                                                            if (App::getLocale() != env('DEFAULT_LANGUAGE')) {
                                                            $Category_link_url = url(App::getLocale()."/" .$SubMnuCategory->$slug_var);
                                                            }else{
                                                            $Category_link_url = url($SubMnuCategory->$slug_var);
                                                            }
                                                            } else {
                                                            if (App::getLocale() != env('DEFAULT_LANGUAGE')) {
                                                            $Category_link_url = route('FrontendTopicsByCatWithLang', ["lang"=>App::getLocale(),"section" => $subMenu->webmasterSection->name, "cat" => $SubMnuCategory->id]);
                                                            }else{
                                            ...
                                            <ul class="dropdown-menu">
                                                @foreach($subMenu->webmasterSection->topics as $SubMnuTopic)
                                                    @if($SubMnuTopic->expire_date =='' || ($SubMnuTopic->expire_date !='' && $SubMnuTopic->expire_date >= date("Y-m-d")))
                                                        <li>
                                                            <?php
                                                            if ($SubMnuTopic->$slug_var != "" && \App\Helpers\Helper::GeneralWebmasterSettings("links_status")) {
                                                            if (App::getLocale() != env('DEFAULT_LANGUAGE')) {
                                                            $topic_link_url = url(App::getLocale()."/" .$SubMnuTopic->$slug_var);
                                                            }else{
                                                            $topic_link_url = url($SubMnuTopic->$slug_var);
                                                            }
                                                            } else {
                                                            if (App::getLocale() != env('DEFAULT_LANGUAGE')) {
                                                            $topic_link_url = route('FrontendTopicByLang',["lang"=>App::getLocale(),"id"=>$SubMnuTopic->id]);
                                                            }else{
                                ...
                                @elseif($subMenu->type==2)
                                    ...
                                    <li>
                                        <?php
                                        if ($subMenu->webmasterSection->$slug_var != "" && \App\Helpers\Helper::GeneralWebmasterSettings("links_status")) {
                                        if (App::getLocale() != env('DEFAULT_LANGUAGE')) {
                                        $mmnnuu_link = url(App::getLocale()."/" .$subMenu->webmasterSection->$slug_var);
                                        }else{
                                        $mmnnuu_link = url($subMenu->webmasterSection->$slug_var);
                                        }
                                        }else{
                                        if (App::getLocale() != env('DEFAULT_LANGUAGE')) {
                                        $mmnnuu_link =url(App::getLocale()."/" .$subMenu->webmasterSection->name);
                                        }else{
