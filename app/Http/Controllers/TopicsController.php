...
class TopicsController extends Controller
{
    ...
    public function store(Request $request, $webmasterId)
    {
        ...
        if ($WebmasterSection) {
            //
            if (@$request->name_taxes != "" && $request->number_tax != "") {
                $this->validate($request, [
                    'name_taxes' => 'required|unique:topics',
                    'number_tax' => 'required|unique:topics',
                ]);
            }elseif (@$request->percent_org != "" && $request->percent_delivery != ""){
                $this->validate($request, [
                    'percent_org' => 'required|unique:topics',
                    'percent_delivery' => 'required|unique:topics',
                ]);
            }elseif(@$request->color_stand){
           $title=Topic::where("color_stand",$request->color_stand)->where("title_il",$request->title_il)->Where("title_en",$request->title_en)->get();
            // dd($request->color_stand,$title);
            if(!$title->isEmpty()){
                
                $this->validate($request, [
                    'title_il' => 'required|unique:topics',
                    'title_en' => 'required|unique:topics',
                ]);
            }
            }elseif(@$request->size){
                 $this->validate($request, [
                    'size' => 'required|unique:topics',
                ]);
            }elseif($webmasterId == 22){
                $this->validate($request, [
                    'photo_file' => 'required|mimes:png,jpeg,jpg,gif|max:3000',
                ]);
            }elseif($webmasterId == 24){
                $this->validate($request, [
                    // 'photo_file' => 'required|mimes:png,jpeg,jpg,gif|max:3000',
                ]);
            }elseif($webmasterId == 25){
                $this->validate($request, [
                    // 'photo_file' => 'required|mimes:png,jpeg,jpg,gif|max:3000',
                ]);
            }elseif($webmasterId == 26){
                $this->validate($request, [
                    // 'photo_file' => 'required|mimes:png,jpeg,jpg,gif|max:3000',
                ]);
            }elseif($webmasterId == 23){
                $this->validate($request, [
                    'photo_file' => 'required|mimes:png,jpeg,jpg,gif|max:3000',
                ]);
            }
            ...
            if (@$request->color_stand != "") {
                $Topic->color_stand = $request->color_stand;
            }
            if (@$request->name_taxes != "") {
                $Topic->name_taxes = $request->name_taxes;
            }
            if (@$request->percent_org != "") {
                $Topic->percent_org = $request->percent_org;
            }
            if (@$request->percent_delivery != "") {
                $Topic->percent_delivery = $request->percent_delivery;
            }
            if (@$request->number_tax != "") {
                $Topic->number_tax = $request->number_tax;
            }
            if ($fileFinalName != "") {
            ...
            // Meta title
            $Topic->seo_title_il = $request->title_il;
            $Topic->seo_title_en = $request->title_en;

            // URL Slugs
            $slugs = \App\Helpers\Helper::URLSlug($request->title_il, $request->title_en, "topic", 0);
            ...
            if ($WebmasterSection->customFields) {
            ...
            return redirect()->action('TopicsController@index', [$webmasterId, $Topic->id])->with('doneMessage',
    ...
    public function update(Request $request, $webmasterId, $id)
    {
        ...
        if ($WebmasterSection) {
            ...
            if ($Topic) {
                ...
                $Topic->title_il = $request->title_il;
                $Topic->title_en = $request->title_en;
                $Topic->details_ar = $request->details_il;
                $Topic->color = $request->color;
            $Topic->size = $request->size;
                $Topic->details_en = $request->details_en;
                $Topic->date = $request->date;
...
//                dd($Topic);
                ...
                return redirect()->action('TopicsController@index', [$webmasterId, $id])->with('doneMessage',
    ...
    public function updateAll(Request $request, $webmasterId)
    {
        ...
        if ($WebmasterSection) {
            ...
            if ($request->action == "order") {
                ...
                //  dd($request->ids);
                // Delete photo files
                $PhotoFiles = Photo::wherein('id', $request->ids)->get();
                foreach ($PhotoFiles as $PhotoFile) {
                    if ($PhotoFile->file != "") {
                        File::delete($this->getUploadPath() . $PhotoFile->file);
                    }
    ...
    function seo(Request $request, $webmasterId, $id)
    {
        ...
        if ($WebmasterSection) {
            ...
            if ($Topic) {
                ...
                $Topic->seo_title_il = $request->seo_title_il;
                ...
                //URL Slugs
                $slugs = \App\Helpers\Helper::URLSlug($request->seo_url_slug_ar, $request->seo_url_slug_en, "topic", $id);
                $Topic->seo_url_slug_ar = $slugs['slug_ar'];
                $Topic->seo_url_slug_en = $slugs['slug_en'];
    ...
    public
    function photos(Request $request, $id)
    {
            //
            $this->validate($request, [
                'file' => 'image|max:3000',
    ...
    function filesStore(Request $request, $webmasterId, $id)
    {
        ...
        if ($WebmasterSection) {
            ...
            if ($fileFinalName != "") {
                ...
                $AttachFile->title_il = $request->title_il;
